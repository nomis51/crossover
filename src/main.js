...
const { onWindowChanged } = require('./window-detection.js');
const detectorInterval = onWindowChanged((error, infos) => {
	if (error) {
		console.error(error);
		return;
	}
	...
	console.log(`Process path: ${infos.processPath}`);
});
// To stop the detection interval
